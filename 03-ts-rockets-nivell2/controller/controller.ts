...
const formRocket = document.getElementById('formRocket') as HTMLFormElement,
      btnRocket1 = document.getElementById('btnRocket1') as HTMLButtonElement, 
      btnRocket2 = document.getElementById('btnRocket2') as HTMLButtonElement, 
      inThruster = document.getElementById('inputThruster') as HTMLDivElement,
      code = document.getElementById('code') as HTMLInputElement,
      thrusternum = document.getElementById('thrusterNum') as HTMLInputElement,
      btnThruster = document.getElementById('buttonThrusters') as HTMLInputElement,
      submitForm = document.getElementById('submitForm1') as HTMLInputElement,
      result = document.getElementById('result') as HTMLButtonElement,
      ...
      instructions = document.querySelector('.instructions') as HTMLDivElement,
...
btnRocket1.addEventListener('click', (e:Event)=>{
...
btnRocket2.addEventListener('click', (e:Event)=>{
  instructions.classList.add('d-none');
  formRocket.classList.remove('d-none');
...
let inPower:HTMLInputElement;
let pressBTN = false;

btnThruster.onclick = function(e:Event){
...
formRocket.addEventListener('submit', (e:Event) => {
  e.preventDefault();
...
function createPowerInputs(inThruster:HTMLDivElement){

  for(var i = 0; i < rocket.thrusterNum; i++){
    const inPower = <HTMLInputElement>document.createElement('input');
...
function removeInputs(inThruster:HTMLDivElement){
...
function addingThrusters(){
  ...
  for(let i =0; i< powerList.length; i++){
    const powerValue = powerList[i] as HTMLInputElement;
...
function validationForm1(code:HTMLInputElement, thrusternum:HTMLInputElement){
  var acumErrores = 0;
  formRocket.classList.remove('is-invalid');
  
  const erCode = document.querySelector('.error-code') as HTMLInputElement,
        erThrusterNum = document.querySelector('.error-thruster') as HTMLInputElement;
...
function validationThruster(){
  ...
  for(let i = 0; i< powerList.length; i++){
        const inPower = powerList[i] as HTMLInputElement;

    if(inPower.value == ''){
      inPower.classList.add('is-invalid');
      const errPower = inPower.nextSibling as HTMLInputElement;
      console.log(errPower.textContent);
      errPower.textContent as string;
...
//regex
function regexCode(code:string){
  var regex = /^[a-zA-Z0-9]{8}$/g;
  return regex.test(code) ? true: false;
}

function regexThrusterNunber(thrusternum:string){
  var regex = /^[1-6]$/g;
  return regex.test(thrusternum) ? true: false;
  
}

function regexPower(iPower:string){
  var regex = /[1-9](0)/g;
...
//BLURevent
formRocket.addEventListener('blur', (event:Event) => {
  const target = event.target as HTMLInputElement
  if(target.value != '') target.classList.remove('is-invalid');
